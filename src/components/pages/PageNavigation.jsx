...
function PageManagerPopover({ pages, activePage, setActivePage, setEditingPage, pageSettings, pagesConfig, persistConfig, setShowAddPageModal, onClose, t }) {
  ...
  return (
    <div
      ref={ref}
      className={`absolute top-full left-0 z-50 mt-2 w-52 origin-top-left transform rounded-2xl border border-white/10 p-1.5 shadow-2xl transition-all duration-200 translate-y-0 scale-100 opacity-100`}
    ...
    >
        ...
        {pages.map((page, idx) => {
          ...
          return (
            <div
              ...
              onDrop={() => { if (dragIdx !== null) movePage(dragIdx, idx); setDragIdx(null); setOverIdx(null); }}
            ...
            >
              <button
                type="button"
                onClick={() => { setActivePage(page.id); onClose(); }}
                className="flex min-w-0 flex-1 items-center gap-2.5"
              >
                <div className={`shrink-0 rounded-md p-1.5 transition-colors ${
                  isActive
                ...
                <div className="min-w-0">
                  <p className={`truncate text-xs font-semibold ${
                    isActive ? 'text-[var(--text-primary)]' : 'text-[var(--text-primary)] group-hover:text-white'
                  }`}>{label}</p>
                  {isHidden && (
                    <p className="text-[9px] uppercase tracking-wider text-[var(--text-muted)]">{t('nav.hidden') || 'Hidden'}</p>
                  )}
                </div>
              </button>
              <button
                type="button"
                onClick={() => { setEditingPage(page.id); onClose(); }}
                className="shrink-0 rounded-md p-1 text-[var(--accent-color)] opacity-0 transition-opacity hover:bg-white/10 group-hover:opacity-100"
      ...
      <button
        type="button"
        onClick={() => { setShowAddPageModal(true); onClose(); }}
...
export default function PageNavigation({
  ...
  useEffect(() => { if (!editMode) setShowManager(false); }, [editMode]);
      ...
      {editMode && (
        <div className="relative shrink-0 pb-2">
          ...
          >
            ...
            <ChevronDown className={`h-3 w-3 transition-transform ${showManager ? 'rotate-180' : ''}`} />
          </button>
