...
export default function EditCardModal({
  ...
  const energyMatch = React.useMemo(() => matchEnergyEntities(entities || {}), [entities]);
  ...
  const tempSensorOptions = React.useMemo(
    () =>
      byDomain('sensor').filter((id) => {
        const entity = entities[id];
        const deviceClass = entity?.attributes?.device_class;
        return deviceClass === 'temperature' || id.includes('temp');
      }),
    [byDomain, entities]
  );
  const humiditySensorOptions = React.useMemo(
    () =>
      byDomain('sensor').filter((id) => {
        const entity = entities[id];
